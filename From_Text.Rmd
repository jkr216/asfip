...
fluidRow(
  column(6, textInput("stock2", "Stock 2", "SPY")),
  column(5, numericInput("w2", "Portf. %", 25, min=1, max =100))
  )

fluidRow(
  column(6, textInput("stock3", "Stock 3", "SPY")),
  column(5, numericInput("w3", "Portf. %", 25, min=1, max =100)))

fluidRow(
  column(6, textInput("stock4", "Stock 4", "SPY")),
  column(5, numericInput("w4", "Portf. %", 25, min=1, max =100)))

fluidRow(
  column(6, textInput("stock5", "Stock 5", "SPY")),
  column(5, numericInput("w5", "Portf. %", 25, min=1, max =100)))

fluidRow(
  column(7, dateInput("date", "Starting Date", "2013-01-01",
                      format = "yyyy-mm-dd")))
...
renderPlot({
  portfolio_returns_byhand() %>%
    ggplot(aes(x = returns)) +
    geom_density(size = 1, fill = "cornflowerblue")
})
```
